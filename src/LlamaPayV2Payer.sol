...
contract LlamaPayV2Payer {
    ...
    function withdraw(uint _id, uint _amount) public {
        ...
        if (token.balance >= paidSinceUpdate) {
            tokens[stream.token].balance -= paidSinceUpdate;
            tokens[stream.token].lastPayerUpdate = uint40(block.timestamp);
        } else {
            uint timePaid = token.balance / token.totalPaidPerSec;
            tokens[stream.token].lastPayerUpdate += uint40(timePaid);
            tokens[stream.token].balance = token.balance % token.totalPaidPerSec;
        }

        // Update stream
        uint available = (token.lastPayerUpdate - stream.lastUpdate) * stream.amountPerSec;
        require(available >= _amount, "amount > balance");
        tokens[stream.token].balance -= _amount;
        ...
        tokens[stream.token].balance += split * (10 ** (20 - decimals));
    }
    ...
    function createStream(address _token, address _vaultToken, address _payee, uint216 _amountPerSec) external {
        ...
        tokens[_token].balance -= delta * tokens[_token].totalPaidPerSec;
        tokens[_token].totalPaidPerSec += _amountPerSec;
        tokens[_token].lastPayerUpdate = uint40(block.timestamp);
    ...
    function cancelStream(uint _id) external {
        ...
        tokens[streams[_id].token].totalPaidPerSec -= streams[_id].amountPerSec;
    }
